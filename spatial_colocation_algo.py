...
df=pd.read_csv("crime_data_small.csv")
...
def preprocess_df(df,crime):
    for i in crime:
        suffix=1
        for j in range(len(df)):
            if i==df["Primary Type"][j]:
                df["Primary Type"][j]=df["Primary Type"][j]+str(suffix)
                suffix+=1
    df=df.rename(columns={"Primary Type":"Event"})
    return df
        ...
        for j in range(i+1,len(size_k_m_1_patterns)):
            ...
            for k in range(len(pattern1_instances)):
                for l in range(len(pattern2_instances)):
                    x1={e for e in pattern1_instances[k]}
                    x2={e for e in pattern2_instances[l]}
...
def spatial_colocation_miner(df, PARTICIPATION_THREOD = 0.2, d_threshold = 20):
    crime=extract_event_types(df)
    df=preprocess_df(df,crime)
    all_patterns=[]
...
spatial_colocation_patterns=spatial_colocation_miner(df, PARTICIPATION_THREOD = 0.2, d_threshold = 20)
print(spatial_colocation_patterns)